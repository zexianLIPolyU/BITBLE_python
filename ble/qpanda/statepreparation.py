...
from ble.qpanda import qgates
from ble import anglecompute, tools
import numpy as np
...
def state_preparation(state, target_qubits, is_real=False, control_qubits=None, control_states=None):
    ...
    Args:
        ...
        target_qubits (list[Qubit] or Qubit): The list of target qubits on which the state will be prepared.
        is_real (bool, optional): Whether the state is real-valued. Defaults to False.
        control_qubits (list[Qubit] or Qubit, optional): Additional control qubits for conditional state preparation. Defaults to None.
        control_states (list[int] or int, optional): The control states for the control qubits (0 or 1). Defaults to None.

    Returns:
        QCircuit: The quantum circuit that prepares the desired state.
    ...
    for layer in range(1, num_qubits):
        for i in range(2 ** layer):
            qgates.qgate('RY',
                         ...
                         control_qubits=[target_qubits[i] for i in list(range(layer))],
    ...
    query_cir = QCircuit()
    query_cir << X(target_qubits[0]) << X(target_qubits[0])
    replace_cir = QCircuit()
    replace_cir << I(target_qubits[0])
...
def compressed_state_preparation(state, target_qubits, is_real=False, epsilon=0, control_qubits=None,
    ...
    Args:
        state (numpy.array): The input state vector to be prepared.
        target_qubits (list[Qubit] or Qubit): The list of target qubits on which the state will be prepared.
            The type of each qubit should be `pyqpanda.pyQPanda.Qubit`.
        ...
        control_qubits (list[Qubit] or Qubit, optional): Additional control qubits for conditional state preparation.
            The type of each qubit should be `pyqpanda.pyQPanda.Qubit`. Defaults to None.
        control_states (list[int] or int, optional): The control states for the control qubits (0 or 1). Defaults to None.

    Returns:
        QCircuit: The compressed quantum circuit that prepares the desired state.
    ...
    for layer in range(1, num_qubits):
        ...
        circuit_ur = qgates.compressed_uniformly_rotation('RY',
                                                          target_qubit=target_qubits[layer],
    ...
    if not is_real:
        ...
        for layer in range(1, num_qubits):
            ...
            circuit_ur = qgates.compressed_uniformly_rotation('RZ',
                                                              target_qubit=target_qubits[layer],
                                                              control_qubits=target_qubits[:layer],
                                                              rotation_angles=ur_angles,
                                                              epsilon=epsilon)
