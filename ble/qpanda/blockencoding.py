...
def qcircuit(matrix, target_qubits, is_real=False, control_qubits=None, control_states=None):
    ...
    Args:
        ...
        target_qubits (list[Qubit] or Qubit): The list of target qubits used to implement the matrix.
        is_real (bool, optional): Whether the matrix is real-valued. If True, only RY gates are used. Defaults to False.
        control_qubits (list[Qubit] or Qubit, optional): Additional control qubits for conditional application of the circuit. Defaults to None.
        control_states (list[int] or int, optional): The control states for the control qubits (0 or 1). Defaults to None.

    Returns:
        QCircuit: The quantum circuit that block encodes the given matrix.
    """
    ...
    for layer in range(n):
        for i in range(2 ** layer):
            qgates.qgate('RY',
                         ...
                         rotation_angle=norm_angles[angle_index + i, -1])
        angle_index += 2 ** layer
    circuit_GLOBAL = circuit_GLOBAL.dagger()
...
def compress_qcircuit(matrix, target_qubits, is_real=False, epsilon=0, control_qubits=None, control_states=None):
    ...
    Args:
        ...
        target_qubits (list[Qubit] or Qubit): The list of target qubits used to implement the matrix. The type of each qubit should be `pyqpanda.pyQPanda.Qubit`.
        is_real (bool, optional): Whether the matrix is real-valued. If True, only RY gates are used. Defaults to False.
    ...
    Returns:
        QCircuit: The compressed quantum circuit that block encodes the given matrix.
    """
    # Create a circuit object
    circuit = QCircuit()
    ...
    circuit_PREP = QCircuit()
    if not is_real:
        ur_circuit = qgates.compressed_uniformly_rotation('RZ',
                                                          target_qubit=target_qubits[0],
                                                          control_qubits=target_qubits[n: 2 ** n],
    ...
    for layer in range(n):
        ...
        ur_circuit = qgates.compressed_uniformly_rotation('RY',
                                                          target_qubit=target_qubits[layer],
                                                          control_qubits=target_qubits[:layer] + target_qubits[n: 2 * n],
